...
每个数据位发生传输错误的概率为5%
模拟接收段情况
计算后可知 可以用（10,6）线性分组码编码
并非汉明码 4个冗余位刚好满足1位分辨本领
 by Haoyu Li*/
...
void HanmingCoding(int a[]){//进行汉明编码
a[N]=(a[0]+a[2]+a[4])%2;
a[N+1]=(a[0]+a[3]+a[4]+a[5])%2;
a[N+2]=(a[1]+a[3]+a[4]+a[5])%2;
a[N+3]=(a[1]+a[2]+a[5])%2;
}
...
void ExamHanming(int a[],int &sign){//利用校验矩阵H进行校验
    sign=1;
int s[r]={0};
s[0]=(a[N]+a[0]+a[2]+a[4])%2;
s[1]=(a[0]+a[3]+a[4]+a[5]+a[N+1])%2;
s[2]=(a[1]+a[3]+a[4]+a[5]+a[N+2])%2;
s[3]=(a[1]+a[2]+a[5]+a[N+3])%2;
